...
#include "DestructibleGroundItem.h"

#include "DestructionForce.h"
#include "GeometryCollection/GeometryCollectionComponent.h"

ADestructibleGroundItem::ADestructibleGroundItem()
{
	...
	if (HasAuthority())
	{
		if (UWorld* World = GetWorld())
		{
			...
			World->GetTimerManager().SetTimer(
				DelayHandle,                       // 记录这个定时器
				this,                                  // 回调目标对象
				&ADestructibleGroundItem::OnDelayFinished,   // 回调函数
				8.f,                                  // Delay 时长（秒）
...
void ADestructibleGroundItem::OnDelayFinished()
{
	Destroy();
}
